...
class REDSTrainDataset(data.Dataset):
    def __init__(self, opt):
        ...
        self.transform = transforms.Compose([DataAug(), ToTensor()])
    ...
    def __getitem__(self, _):
        ...
        center_index = random.randint(half_frame_num, len(self.img_list) - (half_frame_num + 1))

        frames_hr_name = '{}{}'.format(hr_folder_path, self.img_list[center_index])
        frames_hr = cv2.imread(frames_hr_name)
        h, w, ch = frames_hr.shape

        frames_lr = np.zeros((self.frame, int(h / self.scale), int(w / self.scale), ch))
        for j in range(center_index - half_frame_num, center_index + half_frame_num + 1):
            i = j - center_index + half_frame_num
